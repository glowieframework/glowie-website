...
You can also store views inside subfolders for better organization. Just remember to include the folder name as well while rendering a view file (see next).
...
You can pass any parameter to a view while rendering it. To do that, use `$this->renderView()` and pass as the second parameter an associative array of parameters, with the variable name being the key and the value being the value.

This is useful to render views with dynamic content that relies on this variables (see example).
